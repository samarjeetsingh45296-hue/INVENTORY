 ...
 *     --email someone@example.com --role ADMIN [--deactivate]
 *
 ...
 *
 * Refuses to revoke the last active ADMIN, because an installation with
...
async function main(): Promise<void> {
  ...
  if (!email || !roleKey) {
    console.error('Usage: revoke:role -- --email someone@example.com --role ADMIN [--deactivate]');
    process.exit(1);
  }
  ...
  if (roleKey === 'ADMIN') {
    ...
    if (remaining === 0) {
      console.error(
        `Refusing: ${email} is the last active Admin.\n` +
          'Grant ADMIN to another account first:\n' +
          '  pnpm --filter @inventory/api create:user -- --email you@example.com --role ADMIN',
      );
      process.exit(1);
    }
    console.log(`${remaining} other active Admin account(s) will remain.`);
  }
